...
public class DispatcherServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		System.out.println(">> "+siteRootRealPath);
		String configFilePath=siteRootRealPath+configFile;
		try {
			/*
			 * 설정파일(guest_controller_mapping.properties)을 읽어서 Properties 객체생성 
			 */
			...
			*/
			...
			System.out.println("-------설정파일["+configFile+"]을 이용해서 handlerMapping 객체 생성-------");
			while (commandKeyIterator.hasNext()) {
				 ...
				 * Controller 클래스 이름을 사용해서 Controller 객체 생성
				 * 		1. Controller 클래스 이름을 사용해서 클래스를 메모리에 로딩
				 * 		2. 메모리에 로딩된 클래스의 기본생성자를 호출해서 객체 생성
				 */
				Class controllerClass=Class.forName(controllerClassName);
				Controller controllerObject= (Controller)controllerClass.newInstance();	//기본생성자호출코드-->자동으로 불러와지는 클래스는 기본생성자를 public으로 꼭 가지고 있어야 하는 이유
